...
def harris_edge_save(reference_image_path, out_path='harris.png',
                     blockSize=2, ksize=3, k=0.04, threshold_ratio=0.01):
...
def align_images(image_to_align_path, reference_image_path,
    ...
    if method_lower == 'sift':
        if hasattr(cv2, 'SIFT_create'):
            ...
            print("SIFT unavailable, fallback to ORB.")
        ...
        try:
            ...
            print("SURF unavailable, fallback to ORB.")
            detector = cv2.ORB_create(nfeatures=max_features)
            method_lower = 'orb'
    ...
    if des1 is None or des2 is None or len(kp1) < 4 or len(kp2) < 4:
        raise RuntimeError(f"{method} failed: insufficient keypoints or descriptors.")
    ...
    print(f"[{method.upper()}] Total matches: {len(matches)}, keeping {num_good} good.")

    # Extract additional points
    points_ref = np.float32([kp1[m.queryIdx].pt for m in good_matches])
    points_img = np.float32([kp2[m.trainIdx].pt for m in good_matches])
    ...
    print(f"[{method.upper()}] Saved: {out_aligned}, {out_matches}")
...
def make_pdf_from_images(image_paths, out_pdf='assignment4_output.pdf'):
    ...
    for p in image_paths:
        if not os.path.exists(p):
            raise FileNotFoundError(f"{p} missing for PDF.")
        im = Image.open(p)
        if im.mode != "RGB":
            im = im.convert("RGB")
    ...
    for m, f, g in methods:
        try:
            ...
            align_images(
                ...
                method=m,
                max_features=f,
                good_match_percent=g,
                out_aligned=aligned_path,
                out_matches=matches_path
            )
